...
class Poisson(Exponential):
    def sample(self, params, size=1, with_grad=False):
        if with_grad:
            ...
            dist = torch.distributions.poisson.Poisson(self._as_rate(params))
    ...
    def log_prob(self, params, samples):
        return torch.distributions.Poisson(self._as_rate(params)).log_prob(samples).sum(dim=-1).to(self.device)
