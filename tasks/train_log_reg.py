...
def train_log_reg(seed, results_dir, dataset, device, param_dims, epochs, sample_size,
    ...
    model = models.logistic_regression.LinearProbabilistic(param_dims, estimator)
    trainer = train.logistic_regression.LogisticRegression(model, data_holder, device, torch.optim.Adam, learning_rate)
    ...
    for epoch in range(1, epochs + 1):
        train_loss, test_loss = trainer.train_epoch()
