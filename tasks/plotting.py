...
def plot_losses(out_dir, train_losses, test_losses, logscale):
    ...
    for task, train_losses in train_losses.items():
        __plot(train_losses, path.join(out_dir, task, f'train.svg'), logscale)
    for task, test_losses in test_losses.items():
        __plot(test_losses, path.join(out_dir, task,  f'test.svg'), logscale)


def __plot(losses, file_name, logscale):
    if logscale:
        plt.yscale('log')
    for config, loss in losses:
        plt.plot(loss.numpy(), label=config.get('plot_label', config['mc_estimator']))
    plt.legend()
