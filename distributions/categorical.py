...
class Categorical(Distribution):
    def __init__(self, *args, gumbel_temperature: float = 1., **kwargs):
    ...
    def mvsample(self, size):
        ...
        return torch.arange(self.num_categories, device=self.params.device).unsqueeze(0)
    ...
    def pdf(self):
        return torch.arange(self.num_categories, device=self.params.device), self.params.detach().squeeze(0)
